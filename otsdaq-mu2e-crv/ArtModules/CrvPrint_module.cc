...
{
	std::cout << outputPrefix_ << "Beginning job with debug level (diagLevel_) "
	          << diagLevel_ << std::endl;
}
...
void CrvPrint::analyze(art::Event const& e)
{
	...
	if(diagLevel_ > 0)
	{
		...
		std::cout << outputPrefix_
		          << "Number of fragment handles: " << fragmentHandles.size()
		          << std::endl;
	}
	...
	for(const auto& handle : fragmentHandles)
	{
		...
		if(!handle.isValid() || handle->empty())
		{
			if(diagLevel_ > 0)
			{
				std::cout << outputPrefix_ << "Found invalid or empty handle"
				          << std::endl;
			}
			continue;
		}
		...
		if(handle->front().type() == artdaq::Fragment::ContainerFragmentType)
		{
			...
			{
				std::cout << outputPrefix_
				          << "Container fragments (ContainerFragmentType) in handle: "
			...
			for(const auto& cont : *handle)
			{
				...
				{
					std::cout << outputPrefix_
					          << "Container fragment type: " << contf.fragment_type()
					          << std::endl;
					std::cout << outputPrefix_
					          << "Container block count: " << contf.block_count()
					          << std::endl;
				}
				...
				if(contf.fragment_type() != mu2e::FragmentType::DTCEVT)
				{
					...
					{
						std::cout << outputPrefix_
						          << "Container fragment type is not DTCEVT" << std::endl;
					}
					break;
				}
		...
		{  // If the first object in the handle a single fragment
			...
			{
				std::cout << outputPrefix_
				          << "DTC Event (DTCEVT) fragments in handle: " << handle->size()
				          << std::endl;
			}
		...
		{
			std::cerr << outputPrefix_ << "Handle type '" << handle->front().type()
	...
	for(const auto& frag : fragments)
	{
		try
		{
			...
			if(diagLevel_ > 1)
			{
				std::cout << outputPrefix_ << "Event Window Tag: " << EWT << std::dec
				          << std::endl;
			}
			...
			if(diagLevel_ > 1)
			{
				std::cout << outputPrefix_ << "Subevents count: " << subEventsCount
				          << std::endl;
			...
			for(unsigned int iSubEvent = 0; iSubEvent < subEventsCount; ++iSubEvent)
			{
				...
				if(diagLevel_ > 1)
				{
					std::cout << outputPrefix_ << "---> Subevent [" << iSubEvent
					          << "]:" << std::endl;
					std::cout << outputPrefix_ << "Number of Data Blocks: " << blockCount
					          << std::endl;
				...
				for(size_t iBlock = 0; iBlock < blockCount; ++iBlock)
				{
					...
					if(diagLevel_ > 1)
					{
						std::cout << outputPrefix_ << "---> Block [" << iBlock
						          << "]:" << std::endl;
						std::cout << outputPrefix_
						          << "Packet Count: " << blockheader->GetPacketCount()
						          << std::endl;
						std::cout << outputPrefix_ << blockheader->toJSON() << std::endl;
						std::cout
						    << outputPrefix_ << "Block details:" << std::endl
						    << outputPrefix_
						    << "  Subsystem: " << subsystemToString(subsystem)
						    << std::endl
						    << outputPrefix_
						    << "  Valid: " << (blockheader->isValid() ? "Yes" : "No")
						    << std::endl
						    << outputPrefix_ << "  Version: 0x" << std::hex
						    << (int)blockheader->GetVersion() << std::dec << std::endl
						    << outputPrefix_ << "  DTC ID: " << (int)blockheader->GetID()
						    << std::endl
						    << outputPrefix_ << "  Byte Count: " << block->byteSize
						    << std::endl
						    << outputPrefix_ << "  Event Window Tag: "
						    << blockheader->GetEventWindowTag().GetEventWindowTag(true)
						    << std::endl;  // " (0x"

						if(diagLevel_ > 2)
						{
							for(int iPacket = 0; iPacket < blockheader->GetPacketCount();
							    ++iPacket)
							{
								std::cout << outputPrefix_ << "---> Packet [" << iPacket
								          << "]: "
								          << DTCLib::DTC_DataPacket(
								                 ((uint8_t*)block->blockPointer) +
								                 ((iPacket + 1) * 16))
								                 .toJSON()
								          << std::endl;
							}
					...
					if (blockheader->GetSubsystem() == DTCLib::DTC_Subsystem_CRV) {
					    ...
					    if (diagLevel_ > 0) {
					        std::cout << outputPrefix_ << "*** CRV BLOCK FOUND ***" <<
					std::endl;

					        // Additional info for debugging
					        std::cout << outputPrefix_ << "CRV Block Status: " <<
					(int)blockheader->GetStatus() << std::endl;
					    }
					    ...
					    if (blockheader->isValid()) {
					        ...
					        if (blockheader->GetVersion() == 0x0) {
					            ...
					            if (rocStatus != nullptr && diagLevel_ > 0) {
					                std::cout << outputPrefix_ << "CRV ROC Status Packet:"
					<< std::endl
					                          << outputPrefix_ << "  Controller ID: " <<
					(int)rocStatus->ControllerID << std::endl
					                          << outputPrefix_ << "  Active FEB Flags: "
					<< rocStatus->GetActiveFEBFlags().to_string() << std::endl
					                          << outputPrefix_ << "  Trigger Count: " <<
					rocStatus->TriggerCount << std::endl
					                          << outputPrefix_ << "  Event Window Tag: "
					<< rocStatus->GetEventWindowTag() << std::endl;
					            }

					            // Process CRV hits using the data decoder
					            std::vector<mu2e::CRVDataDecoder::CRVHit> hits =
					crvData.GetCRVHits(iBlock);

					            currentEventCrvHits += hits.size();

					            if (diagLevel_ > 0) {
					                std::cout << outputPrefix_ << "Found " << hits.size()
					<< " CRV hits" << std::endl;
					            }
					            ...
					            if (diagLevel_ > 1 && !hits.empty()) {
					                ...
					                std::cout << outputPrefix_ << "  FEB Channel: " <<
					firstHit.first.febChannel << std::endl
					                          << outputPrefix_ << "  Port Number: " <<
					firstHit.first.portNumber << std::endl
					                          << outputPrefix_ << "  Controller Number: "
					<< firstHit.first.controllerNumber << std::endl
					                          << outputPrefix_ << "  Hit Time: " <<
					firstHit.first.HitTime << std::endl
					                          << outputPrefix_ << "  Num Samples: " <<
					firstHit.first.NumSamples << std::endl;

					                if (!firstHit.second.empty()) {
					                    std::cout << outputPrefix_ << "  First Sample ADC:
					" << firstHit.second.front().ADC << std::endl;
					                }
					            }

					            // Deeper waveform details for even higher diagnostic
					levels if (diagLevel_ > 2 && !hits.empty()) { auto &firstHit =
					hits.front(); std::cout << outputPrefix_ << "Waveform for first hit:"
					<< std::endl; for (size_t i = 0; i < firstHit.second.size(); i++) { if
					(i % 8 == 0) std::cout << "  "; std::cout << std::setw(5) <<
					firstHit.second[i].ADC; if (i % 8 == 7 || i == firstHit.second.size()
					- 1) std::cout << std::endl;
					                }
					            }
					        } else {
					            if (diagLevel_ > 0) {
					                std::cout << outputPrefix_ << "CRV block with
					...
					(int)blockheader->GetVersion() << std::dec << std::endl;
					        ...
					        std::cout << outputPrefix_ << "Invalid CRV block header!" <<
					std::endl;
					    }
					}*/
				}
		...
		catch(const std::exception& e)
		{
			if(diagLevel_ > 0)
			{
				std::cerr << outputPrefix_ << "Error processing fragment: " << e.what()
				          << std::endl;
			}
			continue;
		}
	...
	if(diagLevel_ > 0)
	{
		std::cout << outputPrefix_ << "Event Summary: " << currentEventSubEvents
		          << " SubEvents; " << currentEventBlocks << " Blocks; "
		          << currentEventPackets << " Packets; " << currentEventCrvBlocks
		          << " CRV Blocks; " << currentEventCrvHits << " CRV Hits." << std::endl;
	}
