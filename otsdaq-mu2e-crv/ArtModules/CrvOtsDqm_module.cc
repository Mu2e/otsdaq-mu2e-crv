...
void CrvOtsDqm::beginJob() {
    ...
    if (sendHists_) {
        try {
            ...
            std::cout << outputPrefix_ << "Failed to initialise HistoSender: " 
                     << e.what() << std::endl;
    ...
    if (dummyHist_) { 
        ...
        //h1_channels_ = dir.make<TH1F>("h1_channels", "All FEB channels;Global FEB channel ID;Counts", 1792, -0.5, 1791.5);
    ...
    if (enableHttpServer_) {
        try {
            ...
            std::cout << outputPrefix_ << "Failed to start HTTP server: " << e.what() << std::endl;
...
void CrvOtsDqm::Send() {
    ...
    if (diagLevel_ > 1) {
        std::cout << outputPrefix_ << "Sent histograms to " << address_ << ":" << port_ << std::endl;
    }
}
...
void CrvOtsDqm::startHttpServer() {
    ...
    // Set item defaults
    httpServer_->SetItemField("/", "_monitoring", Form("%f", onlineRefreshPeriodMs_));  // Update period in ms
    httpServer_->SetItemField("/", "_sidebar", "0");
    httpServer_->SetItemField("/", "_drawitem", canvasName_.c_str());       // Set DQM canvas as default item
    httpServer_->SetItemField("/", "_http_cache", "0");  // Disable HTTP caching

    lastRefreshTime_ = std::chrono::steady_clock::now();

    std::cout << outputPrefix_ << "HTTP server running on http://localhost:" << httpPort_ << "/" << std::endl;
}
...
void CrvOtsDqm::analyze(art::Event const& event) {
    ...
    if (dummyHist_) {
        ...
        if (diagLevel_ > 2) {
            std::cout << outputPrefix_ << "Filled dummy histogram with value: " << randomValue << std::endl;
        }
        ...
        if (crvDigisHandle.isValid() && !crvDigisHandle->empty()) {
            ...
            for (const auto& digi : crvDigis) {
                ...
                // lets preserve feb 0, that should not exist but its the FEB default setting
...
void CrvOtsDqm::endJob() {
    if (diagLevel_ > 0) {
        ...
        if (!dummyHist_) {
            ...
            std::cout << outputPrefix_ << "Active FEBs: " << activeFEBs_.size() << std::endl;
            // Print FEBs per ROC
            for (auto& [roc, febs] : rocFEBMap_) {
                std::cout << outputPrefix_ << "ROC " << (int)roc << " has " << febs.size() << " FEBs: ";
                for (auto feb : febs) {
                    std::cout << (int)feb << " ";
                }
                std::cout << std::endl;
