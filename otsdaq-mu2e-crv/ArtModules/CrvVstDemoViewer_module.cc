...
class CrvVstDemoViewer : public art::EDAnalyzer
{
  public:
	 ...
	 * "online_refresh_period_ms" refresh period of canvases in milli seconds, defaults to
	 * 500 ms "sleep_per_event" of not 0, an additional sleep in seconds is added to each
	 * event, useful for running on offline data \endverbatim
	 */
...
static_cast<std::size_t>(ceil(sqrt(id_to_index_.size())));

    // Do trivial check first to avoid multipling max * max -> undefined
    if (id_to_index_.size() > max_num_x_plots_ && id_to_index_.size() > max_num_x_plots_ *
max_num_y_plots_) { num_x_plots_ = max_num_x_plots_; num_y_plots_ = max_num_y_plots_; auto
max     = num_x_plots_ * num_y_plots_; auto it      = id_to_index_.begin(); while (it !=
id_to_index_.end()) { if   (it->second >= max)  it = id_to_index_.erase(it); else
++it;
        }
    }
    ...
    TLOG(TLVL_DEBUG) << "id count: " << id_to_index_.size() << ", num_x_plots_: " <<
num_x_plots_ << " / " << max_num_x_plots_ << ", num_y_plots_: " << num_y_plots_ << " / "
<< max_num_y_plots_;
}
*/
...
{
	// We're going to let ROOT's own garbage collection deal with histograms and
	// Canvases...
...
void CrvVstDemoViewer::analyze(art::Event const& event)
{
	...
	if(diagLevel_ > 1)
	{
		TLOG(TLVL_INFO) << "[CrvVstDemoViewer::analyze] Found nFragments  "
		                << fragments.size();
	}
	...
	for(const auto& frag : fragments)
	{
		...
		if(diagLevel_ > 1)
		{
			TLOG(TLVL_INFO) << eventHeader->toJson() << std::endl
			                << "Subevents count: " << event->GetSubEventCount()
			                << std::endl;
		}
		...
		{  // In future, use GetSubsystemData to only get CRV subevents
			...
			for(size_t bl = 0; bl < subevent.GetDataBlockCount(); ++bl)
			{
				...
				if(diagLevel_ > 1)
				{
					...
					{
						TLOG(TLVL_INFO)
						    << DTCLib::DTC_DataPacket(((uint8_t*)block->blockPointer) +
						                              ((ii + 1) * 16))
						           .toJSON()
						    << std::endl;
					}
					...
					if(blockheader->isValid())
					{
						...
						if(blockheader->GetVersion() == 0x0)
						{
							...
							for(auto& hit : hits)
							{
								...
								//     bookCanvas_();
								// }
								histograms_["channels"]->Fill(
								    hit.first.febChannel);  // one hist per FPGA?
								histograms_["timestamps"]->Fill(hit.first.HitTime);
								hist_updated = true;
								// std::copy(toyPtr->dataBeginADCs(),
								// toyPtr->dataBeginADCs() + total_adc_values,
								// graphs_[fid]->GetY());
